...
public final class FontManager {
    ...
    private static final Map<String, CFontRender> FONT_CACHE = new LinkedHashMap<String, CFontRender>(16, 0.75f, true);

    public static CFontRender getRenderer(String family, float fontSize) {
    ...
    private static CFontRender getRendererForPixelHeight(String family, float targetHeight) {
    ...
    private static CFontRender createHeightMatchedRenderer(Font baseFont, float targetHeight) {
    ...
    private static CFontRender getMinecraftRenderer(float fontSize) {
        ...
        return getCachedRenderer(key, () -> new MinecraftFontAdapter(Minecraft.getMinecraft().fontRendererObj, scale));
    }
    ...
    private static synchronized CFontRender getCachedRenderer(String key, Supplier<CFontRender> rendererSupplier) {
        CFontRender renderer = FONT_CACHE.get(key);
        if (renderer != null) {
            return renderer;
        }
    ...
    private static void trimFontCache() {
        while (FONT_CACHE.size() > MAX_CACHED_RENDERERS) {
            Iterator<Map.Entry<String, CFontRender>> iterator = FONT_CACHE.entrySet().iterator();
            if (!iterator.hasNext()) {
                return;
            }

            Map.Entry<String, CFontRender> eldestEntry = iterator.next();
            iterator.remove();
